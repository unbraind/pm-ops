import type { ExtensionApi, ExtensionModule, CommandHandlerContext } from "@unbrained/pm-cli/sdk/authoring";
...
function selfHostsPmChangelog(pkg: PkgJson | undefined): boolean {
 ...
 */
function isCompletePmListEnvelope<T extends PmItem>(envelope: Record<string, unknown>, items: unknown[]): items is T[] {
  ...
  if (typeof envelope.count !== "number" || !Number.isSafeInteger(envelope.count) || envelope.count !== items.length) return false;
  if (typeof envelope.total !== "number" || !Number.isSafeInteger(envelope.total) || envelope.total !== items.length) return false;
...
function readPmItemList<T extends PmItem>(repoPath: string, queryArgs: readonly string[], cache: Map<string, T[] | null>): T[] | null {
  ...
  if (!isCompletePmListEnvelope<T>(parsed as Record<string, unknown>, items)) return set(null);
  return set(items);
}
...
function checkPmChangelogWired(pkg: PkgJson | undefined): PolicyCheckResult {
  ...
  if (selfHostsPmChangelog(pkg)) return { id: "pm-changelog-wired", severity: "error", pass: true, message: "pm-changelog wired (self-hosted CLI + generation/check scripts)" };
...
function runPolicyCheck(def: PolicyCheckDef, ctx: { repoPath: string; pkg: PkgJson | undefined; items: PmItem[] | null }): PolicyCheckResult {
  ...
  switch (def.id) {
    ...
    case "required-scripts":
      result = checkRequiredScripts(ctx.pkg, (def.params?.scripts as string[]) ?? DEFAULT_REQUIRED_SCRIPTS.map((script) => script === "changelog" && selfHostsPmChangelog(ctx.pkg) ? "changelog:full" : script));
      break;
...
interface RepoStatus {
  ...
  audit_high: number | null;
  outdated_count: number | null;
  /** Clone-local pending merge decision receipts (field-aware driver). */
  pending_receipts: number | null;
}

interface StatusResult {
  repos: RepoStatus[];
  summary: { total: number; ready: number; not_ready: number; total_issues: number; total_pending_receipts: number };
}

/** Assemble the ready-or-not status verdict and metrics for one repository. */
async function collectStatus(repoPath: string): Promise<RepoStatus> {
  if (!existsSync(repoPath)) {
    return {
      path: repoPath,
      name: basename(repoPath),
  ...
  const has_pm_changelog = hasPmChangelogDep(pkg);
  const self_hosts_pm_changelog = selfHostsPmChangelog(pkg);
  if (!has_pm_changelog && !self_hosts_pm_changelog) issues.push("pm-changelog not wired");
  ...
  if (pending_receipts > 0) issues.push(`${pending_receipts} pending merge receipt(s)`);
...
async function collectStatusAll(repos: string[], progress: (msg: string) => void): Promise<StatusResult> {
 ...
 */
async function collectMergeReceiptsRepo(repoPath: string, includeReconciled: boolean): Promise<RepoMergeReceipts> {
  const pkg = readPackageJson(repoPath);
  const name = pkg?.name ?? null;
  ...
  const driver: MergeDriverConfigurationAuditResult | null = pmRoot === undefined
    ? null
    : await auditMergeDriverConfiguration(gitRoot);
  // The fence audit compares the committed block against the active schema's
  // type folders.
  const fence: MergeFenceAuditResult | null = pmRoot === undefined
    ? null
 ...
 */
async function collectMergeReceiptsAll(repos: string[], includeReconciled: boolean, progress: (msg: string) => void): Promise<MergeReceiptsResult> {
  const repoReports: RepoMergeReceipts[] = [];
  for (const repo of repos) {
 ...
 */
function describeDecisionValue(value: unknown): string {
 ...
 */
export function renderMergeReceiptsMarkdown(result: MergeReceiptsResult): string {
  const lines: string[] = [];
  lines.push("# pm-ops merge-receipts");
  lines.push("");
  lines.push(
    `Scanned **${result.summary.total}** repo(s): **${result.summary.total_pending}** pending receipt(s), ` +
  ...
  lines.push("");
  for (const repo of result.repos) {
    if (repo.receipts.length === 0) continue;
    lines.push(`## ${repo.name ?? basename(repo.path)}`);
    lines.push("");
    ...
    for (const receipt of repo.receipts) {
      ...
      const decisions = receipt.decisions.length > 0 ? receipt.decisions : [{ field: "-", retained: "-", discarded: "-" } as const];
...
async function computeRepoMetrics(repo: string, nowMs: number, staleThresholdDays: number): Promise<RepoMetrics> {
...
async function collectMetricsAll(repos: string[], staleThresholdDays: number, progress: (msg: string) => void): Promise<MetricsResult> {
...
function renderMetricsPrometheus(result: MetricsResult): string {
  ...
  const mergePendingSamples: string[] = [];
  const mergeReconciledSamples: string[] = [];
  const mergeDriverSamples: string[] = [];
  const mergeFenceSamples: string[] = [];
...
function serializeResult(structured: unknown, format: OutputFormat, formatter: () => string): string {
...
// ---------------------------------------------------------------------------
// Docstring coverage (fleet-wide lexer-backed gate)
// ---------------------------------------------------------------------------

/** Per-repo slice of a docstring coverage run. */
interface DocstringsRepoResult {
  /** Absolute or resolved repo path, as passed on --repos. */
  readonly repo: string;
  /** Stable repo label (package name or directory basename) for display. */
  readonly name: string;
  /** `.ts` files scanned beneath the repo root. */
  readonly files_scanned: number;
  /** Declarations evaluated against the documented-surface rules. */
  readonly declarations_checked: number;
  /** Number of violations found in this repo. */
  readonly violation_count: number;
  /** Violations, capped for display when a repo is large. */
  readonly violations: readonly DocstringViolation[];
  /** Error message when a repo could not be scanned at all. */
  readonly error?: string;
}

/** Aggregate docstring coverage result across the requested repos. */
interface DocstringsResult {
  /** One entry per repo, in --repos order. */
  readonly repos: readonly DocstringsRepoResult[];
  /** Fleet-wide totals. */
  readonly summary: { readonly total: number; readonly with_violations: number; readonly total_violations: number };
}
...
function renderDocstringsMarkdown(result: DocstringsResult): string {
  const lines: string[] = [
    "# pm-ops docstrings",
    "",
...
export default defineExtension({
  ...
  activate(api: ExtensionApi) {
    ...
    api.registerCommand({
      ...
      description:
        "Audit clone-local merge decision receipts and merge-safety configuration across repos. " +
    ...
    api.registerCommand({
      name: "ops docstrings",
      description:
        "Audit docstring coverage across repos with a lexer-backed, fail-closed analyzer. " +
