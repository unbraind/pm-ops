 ...
 */
const DIVISION_PREVIOUS = new Set<SyntaxKind>([
...
const MEMBER_STARTS = new Set<SyntaxKind>([
  SyntaxKind.Identifier,
 ...
 */
const TYPE_CONTINUES = new Set<SyntaxKind>([
...
/** A scanned token with the offsets needed for JSDoc recovery and line numbers. */
interface Token {
  /** Token kind (a `SyntaxKind` numeric value). */
  readonly kind: SyntaxKind;
  /** Offset where the token text starts (after preceding trivia). */
  readonly start: number;
  /** Offset where the token's leading trivia starts. */
...
const EOF_TOKEN: Token = { kind: SyntaxKind.EndOfFile, start: Number.MAX_SAFE_INTEGER, fullStart: Number.MAX_SAFE_INTEGER, text: "" };
...
function tokenize(text: string): Token[] {
  ...
  const tokens: Token[] = [];
  const group: Array<"brace" | "paren" | "bracket" | "sub"> = [];
  ...
  for (;;) {
    ...
    if (kind === SyntaxKind.TemplateHead || kind === SyntaxKind.TemplateMiddle) {
      ...
      if (top === "sub") {
        ...
        group.pop();
        if (kind === SyntaxKind.TemplateMiddle) group.push("sub");
      } else if (top === "brace") {
        group.pop();
      }
    } else if (kind === SyntaxKind.CloseParenToken) {
      if (group[group.length - 1] === "paren") group.pop();
    } else if (kind === SyntaxKind.CloseBracketToken) {
      if (group[group.length - 1] === "bracket") group.pop();
    }
 ...
 */
function buildLineMap(text: string): Uint32Array {
...
class SourceAnalyzer {
  readonly violations: DocstringViolation[] = [];
  declarationsChecked = 0;
  private i = 0;
  private readonly text: string;
  private readonly file: string;
  private readonly tokens: readonly Token[];
  private readonly lineOf: Uint32Array;

  constructor(text: string, file: string, tokens: readonly Token[], lineOf: Uint32Array) {
    this.text = text;
    this.file = file;
    this.tokens = tokens;
    this.lineOf = lineOf;
  ...
  private peekKind(n: number): SyntaxKind | undefined {
  ...
  private matchingClose(openIdx: number): number {
    let depth = 0;
  ...
  private descendBlock(openIdx: number): void {
    this.i = openIdx + 1;
    this.parseStatements();
    if (this.i < this.tokens.length && this.at(SyntaxKind.CloseBraceToken)) this.i++;
  }
  ...
  private parseStatement(): void {
    ...
    switch (k) {
      ...
      case SyntaxKind.SwitchKeyword:
        this.i++;
        this.skipHeaderParen();
        if (this.at(SyntaxKind.OpenBraceToken)) this.descendBlock(this.i);
        return;
      ...
      case SyntaxKind.DefaultKeyword:
        this.i++;
        if (this.at(SyntaxKind.ColonToken)) this.i++;
        return;
  ...
  private takeNameAndTypeParams(): string | undefined {
    let name: string | undefined;
    if (this.at(SyntaxKind.Identifier)) {
      name = this.cur().text;
      this.i++;
    }
    if (this.at(SyntaxKind.LessThanToken)) this.skipTypeParams();
    return name;
  }
   ...
   */
  private findCallableBody():
    { readonly kind: "body"; readonly openIdx: number } | { readonly kind: "signature" } | { readonly kind: "none" } {
    if (this.at(SyntaxKind.LessThanToken)) this.skipTypeParams();
    if (this.at(SyntaxKind.OpenParenToken)) this.skipGroup();
    return this.findFunctionBody();
  }

  /** Parse a `function` declaration or expression at the current token. */
  private parseFunction(opts: { exported: boolean; defaultExport?: boolean; head: Token }): void {
    this.i++; // function
    if (this.at(SyntaxKind.AsteriskToken)) this.i++; // generator
  ...
  private parseClass(opts: { exported: boolean; defaultExport?: boolean; head: Token }): void {
    this.i++; // class
    const name = this.takeNameAndTypeParams();
    const bodyIdx = this.findClassBody();
    if (opts.exported) this.judge(opts.head, name ?? "default");
    if (bodyIdx >= 0) {
      this.i = bodyIdx + 1;
  ...
  private parseMember(className: string, classExported: boolean): void {
    const head = this.cur();
    while (this.at(SyntaxKind.AtToken)) this.skipDecorator();
    let isPrivate = false;
    let accessor = "";
    for (;;) {
      const k = this.cur().kind;
    ...
    if (k === SyntaxKind.HashToken || k === SyntaxKind.PrivateIdentifier) {
      this.i++;
      if (this.at(SyntaxKind.OpenParenToken) || this.at(SyntaxKind.LessThanToken)) this.skipCallable();
      else this.skipMemberRest();
      return;
    }
    ...
    if (k === SyntaxKind.AsteriskToken) this.i++;
    ...
    if (nameKind === SyntaxKind.Identifier || isKeyword(nameKind)) {
      ...
      if (next === SyntaxKind.OpenParenToken || next === SyntaxKind.LessThanToken) {
        if (next === SyntaxKind.LessThanToken) this.skipTypeParams();
        if (this.at(SyntaxKind.OpenParenToken)) this.skipGroup();
        const body = this.findFunctionBody();
        if (body.kind === "body") {
          if (classExported && !isPrivate) this.judge(head, `${className}.${memberName}`);
          this.descendBlock(body.openIdx);
        }
      } else {
        if (classExported && !isPrivate) this.judge(head, `${className}.${name}`);
        this.skipMemberRest();
      }
      return;
    }
  ...
  private skipDecorator(): void {
    this.i++; // @
    if (this.cur().kind === SyntaxKind.Identifier) {
      this.i++;
      while (this.at(SyntaxKind.DotToken)) {
        this.i++;
        if (this.cur().kind === SyntaxKind.Identifier || isKeyword(this.cur().kind)) this.i++;
      }
    }
    if (this.at(SyntaxKind.OpenParenToken)) this.skipGroup();
  }
   ...
   */
  private findFunctionBody(): { readonly kind: "body"; readonly openIdx: number } | { readonly kind: "signature" } | { readonly kind: "none" } {
    let depth = 0;
    let typeStart = false;
    while (this.i < this.tokens.length) {
      const k = this.cur().kind;
      if (k === SyntaxKind.OpenBraceToken && depth === 0 && !typeStart) return { kind: "body", openIdx: this.i };
  ...
  private findClassBody(): number {
    let depth = 0;
    while (this.i < this.tokens.length) {
      const k = this.cur().kind;
      if (k === SyntaxKind.OpenBraceToken && depth === 0) return this.i;
      if (k === SyntaxKind.SemicolonToken && depth === 0) {
        this.i++;
        return -1;
      }
  ...
  private skipNonExportedInterface(): void {
    this.i++; // interface
    if (this.at(SyntaxKind.Identifier)) this.i++;
    if (this.at(SyntaxKind.LessThanToken)) this.skipTypeParams();
    const bodyIdx = this.findClassBody();
    if (bodyIdx >= 0) this.i = this.matchingClose(bodyIdx) + 1;
    if (this.at(SyntaxKind.SemicolonToken)) this.i++;
  }

  /** Skip a non-exported `type` alias to its terminating `;`. */
  private skipNonExportedTypeAlias(): void {
    this.i++; // type
    if (this.at(SyntaxKind.Identifier)) this.i++;
    if (this.at(SyntaxKind.LessThanToken)) this.skipTypeParams();
    this.skipUntilSemicolon();
  ...
  private parseTypeLike(opts: { exported: boolean; head: Token }): void {
    if (opts.exported) {
      if (this.at(SyntaxKind.Identifier)) {
  ...
  private parseExportedVar(opts: { head: Token }): void {
    for (;;) {
      if (this.at(SyntaxKind.Identifier)) {
        this.judge(opts.head, this.cur().text);
        this.i++;
      ...
      while (this.i < this.tokens.length) {
        if (this.consumeIfAt(SyntaxKind.SemicolonToken)) return;
        if (this.at(SyntaxKind.CloseBraceToken)) return;
        const k = this.cur().kind;
        ...
        if (this.skipGroupIfOpen()) continue;
        this.i++;
      }
  ...
  private skipCallable(): void {
    const body = this.findCallableBody();
    if (body.kind === "body") this.descendBlock(body.openIdx);
  }

  /** Skip a member's trailing type or initializer to its terminating `;`. */
  private skipMemberRest(): void {
    while (this.i < this.tokens.length) {
      if (this.consumeIfAt(SyntaxKind.SemicolonToken)) return;
      if (this.at(SyntaxKind.CloseBraceToken)) return;
      const previous = this.tokens[this.i - 1];
  ...
  private consumeIfAt(kind: SyntaxKind): boolean {
    if (this.at(kind)) {
      this.i++;
      return true;
    }
  ...
  private skipGroupIfOpen(): boolean {
    const k = this.cur().kind;
    if (k === SyntaxKind.OpenBraceToken || k === SyntaxKind.OpenParenToken || k === SyntaxKind.OpenBracketToken) {
      this.skipGroup();
      return true;
  ...
  private skipUntilColon(): void {
    this.skipUntilToken(SyntaxKind.ColonToken);
  }

  /** Skip tokens to `terminator` at group-depth zero, balancing nested groups. */
  private skipUntilToken(terminator: SyntaxKind): void {
    while (this.i < this.tokens.length) {
      if (this.consumeIfAt(terminator)) return;
      if (this.at(SyntaxKind.CloseBraceToken)) return;
      if (this.skipGroupIfOpen()) continue;
      this.i++;
    }
  ...
  private parseFor(): void {
    this.i++;
    if (this.at(SyntaxKind.AwaitKeyword)) this.i++;
    this.skipHeaderParen();
    this.parseBlockOrStatement();
  }

  /** Parse a `do body while (…);`. */
  private parseDoWhile(): void {
    this.i++;
    this.parseBlockOrStatement();
  ...
  private parseBlockOrStatement(): void {
    if (this.at(SyntaxKind.OpenBraceToken)) this.descendBlock(this.i);
   ...
   */
  private skipExpressionStatement(): void {
    while (this.i < this.tokens.length) {
      if (this.consumeIfAt(SyntaxKind.SemicolonToken)) return;
      if (this.at(SyntaxKind.CloseBraceToken)) return;
      if (this.at(SyntaxKind.OpenBraceToken)) {
  ...
  private failClosed(token: Token, reason: string): void {
   ...
   */
  private judge(head: Token, symbol: string): void {
    this.declarationsChecked++;
   ...
   */
  private extractJsdoc(head: Token): string {
 ...
 */
export function analyzeSource(text: string, file: string): SourceAnalysis {
...
export function analyzeDocstringCoverage(options: { root: string; sourceDirs?: readonly string[] }): DocstringReport {
  ...
  if (files.length === 0) {
    throw new Error(
      `docstring coverage: no TypeScript source files found under ${options.root} (scanning zero files cannot pass vacuously)`,
    );
  }
  const reportBase = files.length === 1 && files[0] === options.root ? dirname(options.root) : options.root;
  let declarations = 0;
  const violations: DocstringViolation[] = [];
