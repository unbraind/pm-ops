...
export function pmOnPath(environment: NodeJS.ProcessEnv, platform: NodeJS.Platform): string | null {
  const directories = (environment.PATH ?? "")
    ...
    .map((entry) =>
      platform === "win32" && entry.startsWith('"') && entry.endsWith('"')
        ? entry.slice(1, -1)
  ...
  const extensions = platform === "win32"
    ? (environment.PATHEXT ?? ".COM;.EXE;.BAT;.CMD").split(";").map((entry) => entry.trim()).filter((
      entry,
    ) => entry.length > 0)
    : [""];
 ...
 */
export function runPrepareMergeDriver(
        ...
        ? {
          ...Object.fromEntries(
            Object.entries(environment).filter(([name]) =>
              name.toLowerCase() !== "nodefaultcurrentdirectoryinexepath"
            ),
          ),
