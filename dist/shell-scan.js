 ...
 */
function isBraceBoundary(character) {
...
export function tokenizeCommands(text, depth = 0) {
    ...
    for (const found of [...commands]) {
        ...
        for (const candidate of commandCandidates(found)) {
            ...
            if (name === undefined || !SHELL_EVALUATORS.has(name))
                continue;
 ...
 */
function scanCommandPrefix(command) {
    let index = 0;
    let sawPrefix = false;
    let spawned = false;
 ...
 */
export function spawnedAsCommand(input) {
 ...
 */
export function commandName(input) {
...
export function commandCandidates(input) {
    ...
    if (start < command.length)
        candidates.push(command.slice(start));
    if (start === 0)
        return candidates;
 ...
 */
function foldScalarLines(lines) {
    const folded = [];
    let run = [];
    const flush = () => {
        if (run.length > 0)
 ...
 */
export function dedentRunBlocks(text) {
    ...
    while (index < lines.length) {
        ...
        let content = index + 1;
        while (content < lines.length && BLANK_LINE.test(lines[content]))
            content += 1;
        ...
        while (index < lines.length) {
            ...
            if (!BLANK_LINE.test(line) && !line.startsWith(indent))
                break;
            body.push(line);
            index += 1;
        }
...
function leadingAssignments(line) {
    ...
    if (unterminated)
        return found.map((assignment) => ({ name: assignment.name }));
...
export function scalarAssignmentEvents(segment) {
    ...
    for (let commandIndex = 0; commandIndex < commands.length; commandIndex += 1) {
        ...
        while (["then", "do", "else"].includes(command[index]?.value ?? ""))
            index += 1;
        if (command[index]?.value === "export")
            index += 1;
        const grouped = segment.trimStart().startsWith("(") || segment.trimStart().startsWith("{");
        if (!leadingControl && !grouped && commandIndex === 0)
            continue;
        const token = command[index];
 ...
 */
function heredocOpenersIn(line) {
    const openers = [];
    let single = false;
    let double = false;
    ...
    for (let index = 0; index < line.length; index += 1) {
        ...
        for (; cursor < line.length; cursor += 1) {
            ...
            if (delimiterQuote === undefined && (/\s/.test(part) || /[;&|<>]/.test(part)))
                break;
            delimiter += part;
            consumed = true;
        }
        if (!consumed || delimiterQuote !== undefined)
            continue;
...
function heredocLineStates(lines) {
 ...
 */
export function heredocBodyLines(lines) {
 ...
 */
export function heredocExpansionLines(lines) {
 ...
 */
export function unsetNames(command) {
    ...
    if (words[commandIndex]?.value === "command") {
        commandIndex += 1;
        while (words[commandIndex]?.value.startsWith("-"))
            commandIndex += 1;
    }
    else if (words[commandIndex]?.value === "builtin") {
        commandIndex += 1;
    }
    if (words[commandIndex]?.value !== "unset")
        return [];
    const names = [];
 ...
 */
export function segmentShellLine(line) {
    const segments = [];
    let value = "";
    let quote;
    for (let index = 0; index < line.length; index += 1) {
 ...
 */
export function caseDepthChange(line) {
 ...
 */
export function startsEnclosingCaseArm(line) {
    const bare = unquotedText(line);
    ...
    if (opensCase && /(?:^|[\s;&|])in(?=[\s;&|]|$)/u.test(armText.slice(0, label)))
        return false;
    return true;
}
/**
 ...
 */
export function startsCaseArm(line) {
    const bare = unquotedText(line);
    let groups = 0;
 ...
 */
export function blockDepthChange(line, insideCase = false) {
    const bare = unquotedText(line);
    let depth = 0;
    let open = 0;
 ...
 */
export function shellScalarsByLine(text) {
    const lines = text.split("\n");
 ...
 */
export function shellScalars(text) {
