 ...
 */
function manifestCommandBodies(text) {
    let parsed;
    try {
 ...
 */
export function manifestCommandLines(text) {
...
export function isPublishCommand(command) {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (VALUE_TAKING_FLAGS.has(token.value)) {
            index += 1;
            continue;
        }
        if (token.value.startsWith("-"))
            continue;
        if (RUNNER_SUBCOMMANDS.has(token.value))
            return false;
        if (token.value === "publish")
            return true;
    }
    return false;
}
 ...
 */
export function attestationEnabled(command) {
    const args = commandArguments(command);
    let enabled = false;
    for (let index = 0; index < args.length; index += 1) {
...
function startsIfSiblingArm(segment) {
 ...
 */
export function trackedPublishSources(root) {
...
export function verify(root) {
    const sources = trackedPublishSources(root).map((file) => ({
        file,
 ...
 */
export function report(result, write, exit) {
