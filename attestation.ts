 ...
 */
const leafName = (word: string): string => word.slice(word.lastIndexOf("/") + 1);
...
export function auditPublishAttestation(sources: SourceFile[]): PublishAttestationResult {
  ...
  const { failures, notes } = tallyFlaggedInvocations(
    invocations,
    (invocation) => {
  ...
  if (firstLine.startsWith("#!")) {
    ...
    if (words[index] !== undefined && leafName(words[index]!) === "env") {
      ...
      while (words[index] !== undefined) {
        ...
        if (word.startsWith("-")) {
          index += 1;
          if (ENV_OPTIONS_WITH_OPERAND.has(word)) index += 1;
          continue;
        }
    ...
    if (interpreter !== undefined && SHELL_INTERPRETERS.has(leafName(interpreter))) return true;
  }
